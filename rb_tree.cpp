...
void rb_tree::insert_count(){
    cout<<"Counter for the number of duplicates: "<<dubplicates<<endl;
    cout<<"Counter for each of the insertion cases (red-black tree only): "<<endl;
    cout<<"case 1: "<<case1<<", case 2: "<<case2<<", case3: "<<case3<<endl;
    cout<<"Counter for left rotate and for right rotate (red-black tree only)： "<<endl;
    cout<<"left rotate: "<<leftR<<", right rotate: "<<rightR<<endl;
}
...
//    return index;
//    cout << "size: " << outputarray.size() << "\n";
//    cout << "capacity: " << outputarray.capacity() << "\n";
//    cout << "max_size: " << outputarray.max_size() << "\n";
}
...
void rb_tree::test(rb_tree_node* x,int level){
    if (x != T_nil)
    {
        if (x->color == RB_BLACK){
            test(x->left, level+1);
            test(x->right, level+1);
        }else{
            test(x->left, level);
            test(x->right, level);
        }
